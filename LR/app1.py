...
if Model == 'Uzupełnianie danych':
    ...
    st.subheader('Zakłada się, że obserwowany przebieg zjawiska składa się z części systematycznej (trend, sezonowość) w oparciu, o które buduje się model. Dodatkowo wyróżnia się część przypadkową (szum). Chcąc wyodrębnić wymienione składniki dokonuje się '+r'$\textbf{dekompozycji szeregu czasowego}$')
    st.markdown('---')
